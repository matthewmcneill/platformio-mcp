...
export const BOARD_PATTERNS = [
  ...
  { patterns: [/Wemos.*D1/i, /WEMOS/i], boardType: 'ESP8266', boards: ['d1_mini', 'd1_mini_pro'] },
  { patterns: [/Arduino.*Uno/i], boardType: 'Arduino', boards: ['uno'] },
  { patterns: [/Arduino.*Mega/i], boardType: 'Arduino', boards: ['megaatmega2560'] },
  { patterns: [/Arduino.*Nano/i], boardType: 'Arduino', boards: ['nano'] },
  { patterns: [/Arduino.*Leonardo/i], boardType: 'Arduino', boards: ['leonardo'] },
  { patterns: [/STM32.*Nucleo/i], boardType: 'STM32', boards: ['nucleo_f401re', 'nucleo_f103rb'] },
  { patterns: [/Raspberry.*Pi.*Pico/i, /RP2040/i], boardType: 'RP2040', boards: ['pico'] },
];
 ...
 */
export function mapVidPidToBoard(hwid: string, description: string = ''): string | undefined {
  // Extract standard hex VID:PID
  const vidPidMatch = hwid.match(/VID:PID=([0-9A-Fa-f]{4}):([0-9A-Fa-f]{4})/i) || 
